...
class RearrangementEnv(dm_env.Environment):
    ...
    def __init__(
        ...
        # Comment: margin/gap not currently supported
        # add props
    ...
    def reset(self, qpos) -> dm_env.TimeStep:
        ...
        # TODO: set resetted environment state 
    ...
    def step(self, mjx_data, dummy) -> dm_env.TimeStep:
        """
        Updates the environment according to the action and returns a `TimeStep`.
        """
        
        # set controls 
        ctrl = mjx_data.ctrl.at[0].set(0.0)
...
if __name__=="__main__":
    ...
    # for now create dummy qpos
    qpos = jnp.zeros((400, 8))
    iter = 0
    mjx_data = env.reset(qpos)
    while True:
        mjx_data = env.step(mjx_data, qpos)
        print(iter)
        iter+=1
