"""
An adaptation of dm_control prop_initializer.py to include rejection sampling for objects that are too close each other.

source: https://github.com/google-deepmind/dm_control/blob/main/dm_control/composer/initializers/prop_initializer.py
"""
...
class PropPlacer(composer.Initializer):
  ...
  def _has_collisions_with_prop(self, physics, prop):
    ...
    for contact in contacts:
      # for debugging contact pairs
      #print("***")
      #print("contact.geom1", mj_id2name(physics.model.ptr, 5, contact.geom1))
      #print("contact.geom2", mj_id2name(physics.model.ptr, 5, contact.geom2))
      #print("contact.dist", contact.dist)
        
      # Ignore contacts with the table (this is super unclean need to refactor)
      # technically the PropPlacer initializer has an exclude entity list
      # see this issue: https://github.com/google-deepmind/dm_robotics/issues/20
      if mj_id2name(physics.model.ptr, 5, contact.geom1) == "table/table" or mj_id2name(physics.model.ptr, 5, contact.geom2) == "table/table":
        continue
